...
const Communities = ({ onChats, onGroups }) => {
  const { isDarkMode } = useTheme();
  const filters = ['Unread', 'Active Communities', 'Favourites'];
          ...
          <div key={i} className={`flex items-start justify-between p-3 rounded-lg shadow-sm transition-colors ${isDarkMode ? 'bg-gray-800' : 'bg-gray-100'}`}>
            <div className="flex items-start space-x-3">
              ...
              <div>
                ...
                <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-700'}`}>Latest community post here</p>
